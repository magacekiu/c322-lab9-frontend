...
const host_local = "https://ducks-service-latest-1-shia.onrender.com";

const host_remote = "https://ducks-service-latest-1-shia.onrender.com";
...
async function fetchDucks() {
    const token = getTheToken(); // Retrieve the stored token
    try {
        let response = await fetch(getHost() + "/ducks", {
            method: "GET",
            headers: {
